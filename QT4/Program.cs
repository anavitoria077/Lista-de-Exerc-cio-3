...
class Program
{
    ...
    {
      string? cura;
      int fases, ponto = 0, curapontos = 0, final = 0;
     Console.WriteLine("Quantas fases você deseja jogar?");
     int.TryParse(Console.ReadLine(), out fases);
     for (int i = 0; i < fases; i++)
     {
        Console.WriteLine("Quantos pontos o heroi perdeu e ganhou em cada fase?");
        int.TryParse(Console.ReadLine(), out ponto);
      
      Console.WriteLine("Usou a cura");
      cura = Console.ReadLine();
      if (cura == "Sim")
      { 
        Console.WriteLine("Quantos pontos de cura?");

    int.TryParse(Console.ReadLine(), out curapontos);
      }
      final += curapontos;
      final = ponto+final+100;
}
   Console.WriteLine($"O seu total de pontos é {final}");
}
}